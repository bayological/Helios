...
contract HeliosBS {
    ...
    function addLiquidity(
        ...
        bytes memory data
    ) public payable prank returns (uint256) {
        return
            Helios(target).addLiquidity{value: msg.value}(
                to,
                id,
                token0amount,
                token1amount,
    ...
    function createPair(
        ...
        uint8 fee,
        bytes memory data
    ) public payable prank returns (uint256, uint256) {
        return
            Helios(target).createPair{value: msg.value}(
                to,
                tokenA,
                tokenB,
                tokenAamount,
